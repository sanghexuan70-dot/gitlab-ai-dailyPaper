...
export async function getTodayCommits(config, date = new Date()) {
  ...
  try {
    ...
    console.warn("⚠️  无法获取用户信息,将使用用户名匹配");
  }
  ...
  for (const projectId of projectIds) {
    try {
      ...
      try {
        ...
        console.warn(`⚠️  无法获取项目 ${projectId} 的名称,使用默认名称`);
      }
...
function isPureMergeCommit(title) {
  ...
  return /^(Merge branch|Merge remote-tracking|Merge pull request)\s+\S+\s+into\s+\S+$/i.test(title);
}
